...
export function dbscan(embeddings, epsilon = 0.4, minPoints = 2) {
  ...
  // Log sample distances between first few tabs for debugging
  if (tabIds.length >= 2) {
    console.log('[DBSCAN] ========== SAMPLE DISTANCES ==========');
    const sampleSize = Math.min(5, tabIds.length);
    for (let i = 0; i < sampleSize; i++) {
      for (let j = i + 1; j < sampleSize; j++) {
        const dist = cosineDistance(embeddings[tabIds[i]], embeddings[tabIds[j]]);
        console.log(`[DBSCAN] Distance between tab ${tabIds[i]} and ${tabIds[j]}: ${dist.toFixed(4)}`);
      }
    }
    console.log('[DBSCAN] ==========================================');
  }
  ...
  for (const tabId of tabIds) {
    ...
    neighborCounts.push({ tabId, count: neighbors.length });

    if (neighbors.length < minPoints) {
      // Mark as potential noise (may be added to a cluster later)
      console.log(`[DBSCAN] Tab ${tabId} has only ${neighbors.length} neighbors (need ${minPoints}) - skipping`);
      continue;
    }
  ...
  console.log('[DBSCAN] ========== NEIGHBOR STATISTICS ==========');
  neighborCounts.sort((a, b) => b.count - a.count);
  neighborCounts.slice(0, 10).forEach(({ tabId, count }) => {
    console.log(`[DBSCAN] Tab ${tabId}: ${count} neighbors within epsilon ${epsilon.toFixed(3)}`);
  });
  console.log('[DBSCAN] ============================================');
...
export function suggestEpsilon(embeddings, k = 4) {
  ...
  // Find the "elbow" point - use 25th percentile for much tighter clusters
  // This finds the natural break between tight clusters and outliers
  // Lower percentile = stricter clustering = more distinct, focused groups
  const elbowIndex = Math.floor(distances.length * 0.25);
  const suggestedEpsilon = distances[elbowIndex] || 0.25;

  console.log(`[DBSCAN] Suggested epsilon: ${suggestedEpsilon.toFixed(3)} (25th percentile of k-distances)`);
  console.log(`[DBSCAN] Distance range: min=${distances[0]?.toFixed(3)}, max=${distances[distances.length-1]?.toFixed(3)}`);
